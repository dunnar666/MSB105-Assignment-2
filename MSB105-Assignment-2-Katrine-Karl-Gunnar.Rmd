...
ggplot(heights, mapping = aes(x = height, y = inntekt, col = inntekt)) +
  geom_point()
```
Igjen ser vi at de øverste 2 prosentene ligger langt fra resten av observasjonene. 


```{r boxplot-høyde-inntekt, echo=FALSE}
heightsPosInc <- subset(heights, inntekt < 1000000)
ggplot(heightsPosInc, mapping = aes(x = height_cmInt, y = inntekt)) +
...
ggplot(heightsPosInc, mapping = aes(x = height_cm, y = inntekt)) +
  scale_y_continuous(trans = scales::log_trans()) +
