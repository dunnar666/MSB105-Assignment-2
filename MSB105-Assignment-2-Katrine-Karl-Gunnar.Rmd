...
Det gjenspeiler også den større endringen i plottet over, der vi nå ser en tydeligere diferanse i regresjonslinjene.
...
Vi ser også at vi finner individer i alle aldre jevnt fordelt på forkjellig høyde og inntektsnivå.
...
```{r komplett_reg, echo=T}
summary(lm(inntekt ~ education + height_cm + sex + weight + afqt + marital, data = heights))

summary(lm(inntekt ~ education + height_cm + sex + weight + afqt + marital, data = heights_liminc))
```

Som mistenkt, ser vi at ved å kjøre en regresjon som inkluderer alle 6 variablene i datasettene, så får vi kun en forklaringsgrad på relativt lave 25.44% når vi inkluderer hele settet og 26.55% når vi begrenser inntekten til 1.600.000. 
Dette betyr at det fremdeles er ukjente variabler som spiller en vesentlig større rolle på inntekten enn de variblene som er observert.
