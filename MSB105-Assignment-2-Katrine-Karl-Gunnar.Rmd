...
summary(lm(inntekt ~ height_cm, data = heights_liminc))
```

Her ser vi tydelig at høyde ikke er en særlig relevant faktor for inntekten.
I den første modellen ser vi av *R^2*
...
```{r}
m7 <- ggplot(data = heights, mapping = aes(x = height_cm, y = inntekt)) +
    facet_wrap(~cut_number(education, n =2)) +
  geom_point(aes(colour = cut_number(education, n = 2)), show.legend = F) +
  geom_smooth(method = "lm", se = FALSE)

m8 <- ggplot(data = heights_liminc, mapping = aes(x = height_cm, y = inntekt)) +
    facet_wrap(~cut_number(education, n =2)) +
  geom_point(aes(colour = cut_number(education, n = 2)), show.legend = F) +
  geom_smooth(method = "lm", se = FALSE)
...
ggplot(data = heights_liminc, mapping = aes(x = height_cm, y = inntekt)) +
    facet_wrap(~cut_number(education, n =2)) +
  geom_point(aes(colour = cut_number(education, n = 2))) +
  geom_smooth(method = "lm", se = FALSE)
```
