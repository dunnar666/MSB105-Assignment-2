...
heights_liminc <- heights %>%
  filter(inntekt < 1000000)

m1 <- ggplot(heights, mapping = aes(x = height_cm, y = inntekt, col = inntekt)) +
  geom_point()
m2 <- ggplot(heights_liminc, mapping = aes(x = height_cm, y = inntekt, col = inntekt)) +
  geom_point()
ggarrange(m1,m2)
```
...
Vi ser igjen at alle observasjoner ligger utenfor beregnet standardavvik. 
For å kunne jobbe videre med datasettet så bør disse ekskluderes, da de bidrar til å "forstyrre". 

```{r heightsposinc-jitter-smooth, echo=False}
m5 <- ggplot(heights, mapping = aes(x = height_cm, y = inntekt)) +
  geom_point(position = "jitter") +
  geom_smooth(method = 'lm')

m6 <- ggplot(heights_liminc, mapping = aes(x = height_cm, y = inntekt)) +
  geom_point(position = "jitter") +
  geom_smooth(method = 'lm')
